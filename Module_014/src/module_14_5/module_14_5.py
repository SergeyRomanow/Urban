...
from crud_functions import (
    initiate_db, get_all_products, add_products,
...
# 'Регистрация'.
@dp.message_handler (
        lambda message : message.text.lower ( ) == 'регистрация'
        )
async def sing_up ( message: types.Message ) :
    ...
    # После ожидать ввода возраста в атрибут
    # RegistrationState.username при
    # помощи метода set.
...
# Эта функция должна обновляться данные в состоянии RegistrationState.email
# на message.text.
...
async def set_age ( message: types.Message, state: FSMContext ) :
    ...
    # Далее брать все данные (username, email и age) из состояния и
    # записывать в таблицу Users
...
async def get_formulas ( call: types.CallbackQuery ) :
    ...
    formula_message = (
            ...
            "Для мужчин: 10 * вес (кг) + 6.25 * рост (см) - "
            "5 * возраст (" " лет) + 5\n"
            "Для женщин: 10 * вес (кг) + 6.25 * рост (см) - "
            "5 * возраст (" " лет) - 161"
    )
...
async def get_buying_list ( message: types.Message ) :
    ...
    for product in products :
        long_string = (f"Название: {product [ 1 ]} | "
